    ...
    SYMBOL_TYPE *data, size_t size)
{
    ...
    for (size_t i = 0; i < MAX_CODEWORD_LENGTH; ++i)
    {
        ...
        for (size_t j = 0; j < symbols.size(); ++j)
        {
            ...
            const float numismatic = (float)(symbols[j].second) / (float)(size);
    ...
    std::shared_ptr<std::vector<Symbol>> symbol_length = std::make_shared<std::vector<Symbol>>();
    ...
    std::shared_ptr<std::vector<Symbol>> symbol_lengths)
{
    ...
    if (size % 8 != 0)
        size += 8 - (size % 8);
    size /= 8;
    table.compressed_size = size % sizeof(UNIT_TYPE) == 0 ? size / sizeof(UNIT_TYPE) : (size / sizeof(UNIT_TYPE)) + 1;
    UNIT_TYPE code = 0;
                                             ...
                                             std::shared_ptr<llhuff::LLHuffmanEncoderTable> encoder_table)
{
    ...
    {
        auto code = encoder_table->dict[in[in_unit]] while (at + code.length < max_bits && in_unit < size_in)
        {
            window <<= code.length;
...
{
    std::shared_ptr<cuhd::CUHDCodetable> table = std::make_shared<cuhd::CUHDCodetable>(enc_table->dict.size());
